"""
This package gets user settings (from .env file or environment variables) and creates
...
from pvae.conf import common  # noqa: F401
from pvae.conf.subconf import *  # noqa: F403
...
def generate_env_vars(conf_dict):
    ...
    for var_name, var_value in conf_dict.items():
        ...
        if isinstance(var_value, (str, int, PurePath)):
            # skip local, temporary variables that are not meant to be included
            ...
            if not var_value.__name__.startswith("pvae.conf"):
                continue
